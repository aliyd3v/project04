...
import { createUserSchema, updatePasswordSchema, updateUserSchema } from '../utils/validateSchemas.js'
        ...
        try {
            ...
            if (user.rows[0].role === 'Admin') {
                if (user.rows[0].id !== req.user.id) {
                    return next(
                        new AppError(404, 'fail', 'No acces to change password other admins.'),
                        req,
                        res,
                        next
                    )
            ...
            const passwordHash = await PassController.hash(body.password)
            await pg.query(
